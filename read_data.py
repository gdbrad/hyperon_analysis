...
class InputOutput:
    ...
    def __init__(self,
        ...
        if self.strange == '0':
            ...
            self.dim1_obs = ['m_pi', 'm_k', 'lam_chi', 'eps_pi','m_xi', 'm_xi_st']
    ...
    def _get_bs_data(self):
        ...
        scale_factors = gv.load(self.data_dir +'/scale_setting.p') # on-disk scale data, using this one
        # a_fm =  gv.load(self.data_dir +'/a_fm_results.p') # Nolan's version 
        tmp_afm = {}
        ...
        with h5py.File(self.data_dir+'/input_data.h5','r') as f: 
            for ens in self.ensembles:
                data[ens] = {}
                data[ens]['a_fm'] = to_gvar(f[ens]['a_fm'][scheme][:]) # unnecessary but may be useful for something..
    ...
    def perform_gvar_processing(self):
        ...
        for ens in self.ensembles:
            ...
            for obs in self.dim1_obs+self.y_obs:
                if len(bs_data[ens][obs]) == 5001:
                    print(obs,True)
                    temp[obs] = bs_data[ens][obs][1:] - np.mean(bs_data[ens][obs][1:]) + bs_data[ens][obs][0]
                else:
                    temp[obs] = bs_data[ens][obs]
            temp = gv.dataset.avg_data(temp,bstrap=True)

            for _obs in self.dim1_obs:
                # print(_obs,temp[_obs],bs_data[ens]['units'],temp[_obs]*bs_data[ens]['units_MeV'],'wtf')
                gv_data[ens][_obs] = temp[_obs] *bs_data[ens]['units']
